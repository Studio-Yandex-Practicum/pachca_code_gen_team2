...
class Data(BaseModel):
    ...
    user_status: Optional[User_status] = Field(None, description='Статус. Возвращается как null, если статус не установлен.')
    title: Optional[str] = Field(None, description='Должность')
    created_at: Optional[str] = Field(None, description='Дата создания (ISO-8601, UTC+0) в формате YYYY-MM-DDThh:mm:ss.sssZ')
    time_zone: Optional[str] = Field(None, description='Часовой пояс пользователя')
    image_url: Optional[str] = Field(None, description='Ссылка на скачивание аватарки')


class ResponseGetemployeeGet200(BaseModel):
